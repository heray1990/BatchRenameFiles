...
def rename(outDir, keyType, strStartNum):
    if keyType == 'fairplay':
        strRe = r'*fairplay*'
    ...
    for pathAndFilename in glob.iglob(os.path.join(outDir, strRe)):
        #print('%d' % len(str(intStartNum)))
        ...
        os.rename(pathAndFilename, os.path.join(outDir, strOutputFileName))
